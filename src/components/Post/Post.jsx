...
const Post = ({ post }) => {
  ...
  const handleShowImage = () => {
    confirm({
      title: <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 2 }}>
        ...
        <Typography variant='h6' color='text.primary'>{post?.hashtags?.map((hashtag, index) => (<Link key={index} href="#" ><Typography color="hashtag.primary">#{hashtag}</Typography></Link>))}</Typography>
      </Box>,
      content:
        <Box>
  ...
  return (
    <>
      ...
      >
        <HeadPost userName={post?.author[0]?.username}
          date={`${dayjs(post?.createdAt).format('MMMM DD')} •`}
          avatar={post?.author[0]?.profile_picture} />
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'start', gap: 1 }}>
            {
              post?.hashtags?.map((hashtag, index) => (
                <Link key={index} href="#" >
                  <Typography variant='body1' color="hashtag.primary">
                    #{hashtag}
                  </Typography>
                </Link>
              ))
